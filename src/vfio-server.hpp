...
public:
  ...
  VfioUserServer(std::string sock, int efd,
    ...
    this->vfu_ctx = vfu_create_ctx(VFU_TRANS_SOCK, this->sock.c_str(),
                                   ...
                                   vfu_callback_context, VFU_DEV_TYPE_PCI);
  }
  ...
  int add_regions(std::vector<struct vfio_region_info> regions, int device_fd) {
    ...
    if (regions.size() >
        VFU_PCI_DEV_VGA_REGION_IDX - VFU_PCI_DEV_BAR0_REGION_IDX + 1)
      printf("Warning: got %u mappable regions, \
                        but we expect normal PCI to have %d at most\n",
             (uint)regions.size(),
             VFU_PCI_DEV_VGA_REGION_IDX - VFU_PCI_DEV_BAR0_REGION_IDX + 1);
    // Note, that we only attempt to map bar 0-5
    for (int i = VFU_PCI_DEV_BAR0_REGION_IDX; i <= VFU_PCI_DEV_BAR5_REGION_IDX;
         i++) {
  ...
  int add_irqs(std::vector<struct vfio_irq_info> const irqs) {
    ...
    for (int i = VFU_DEV_INTX_IRQ; i <= VFU_DEV_REQ_IRQ; i++) {
      ...
      ret = vfu_setup_device_nr_irqs(
          this->vfu_ctx, (enum vfu_dev_irq_type)irqs[i].index, irqs[i].count);
      if (ret < 0) {
        die("Cannot set up vfio-user irq (type %d, num %d)", irqs[i].index,
            irqs[i].count);
      }
  ...
  void add_legacy_irq_pollfds(const int intx, const int msi, const int err,
                              const int req) {
    struct epoll_event e;
  ...
  void reset_run_ctx_poll_fd() {
    auto pfd =
        (struct pollfd){.fd = vfu_get_poll_fd(vfu_ctx), .events = POLLIN};
  ...
  void
  setup_passthrough_callbacks(std::shared_ptr<VfioConsumer> callback_context) {
    int ret;
    ...
    ret = vfu_setup_device_dma(this->vfu_ctx, VfioUserServer::dma_register_cb,
    ...
    for (int type = 0; type < VFU_DEV_NUM_IRQS; type++) {
      ...
      ret = vfu_setup_irq_state_callback(
          this->vfu_ctx, (enum vfu_dev_irq_type)type,
   ...
   */
  static bool map_dma_here(vfu_ctx_t *vfu_ctx, VfioUserServer *vfu,
                           vfu_dma_info_t *info, uint32_t *flags_) {
    printf("{ #%lu\n", vfu->mapped.size());
    ...
    int ret = vfu_addr_to_sgl(vfu_ctx, info->iova.iov_base, info->iova.iov_len,
                              sgl, 1, flags);
    if (ret < 0) {
  ...
  static bool unmap_dma_here(vfu_ctx_t *vfu_ctx, VfioUserServer *vfu,
                             vfu_dma_info_t *info) {
    printf("{\n");
  ...
  void *dma_local_addr(uintptr_t dma_address, size_t len) {
    for (const auto &[iova_start_, segment] : this->mappings) {
      ...
      if (iova_start <= dma_address && dma_address < iova_end) {
...
private:
  static int reset_device_cb(vfu_ctx_t *vfu_ctx,
    ...
    std::shared_ptr<VfioUserServer> vfu_ =
        ((VmuxDevice *)vfu_get_private(vfu_ctx))->vfuServer;
    VfioUserServer *vfu =
  ...
  static void intx_state_cb(vfu_ctx_t *vfu_ctx, uint32_t start, uint32_t count,
                            bool mask) {
    std::shared_ptr<VfioUserServer> vfu_ =
        ((VmuxDevice *)vfu_get_private(vfu_ctx))->vfuServer;
    VfioUserServer *vfu =
        vfu_.get(); // lets hope vfu_ stays around until end of this function
                    // and map_dma_here only borrows vfu
    vfu->callback_context->mask_irqs(VFIO_PCI_INTX_IRQ_INDEX, start, count,
                                     mask);
  }

  static void msix_state_cb([[maybe_unused]] vfu_ctx_t *vfu_ctx,
  ...
  int add_region(struct vfio_region_info *region, int device_fd) {
    ...
    ret = vfu_setup_region(this->vfu_ctx, region->index, region->size,
                           &(this->unexpected_access_callback),
