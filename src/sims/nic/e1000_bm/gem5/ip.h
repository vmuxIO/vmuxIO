...
struct ip_opt_data_rr {
        ...
        /* uint32_t	iplist __flexarr; /1* list of IP addresses *1/ */
        uint32_t	iplist; /* list of IP addresses */
    // TODO(HE7086): flexarr
} __attribute__((__packed__));
...
struct ip_opt_data_ts {
        ...
        uint8_t		flg:4,
                        oflw:4;
#endif
        /* uint32_t	ipts __flexarr;	/1* IP address [/ timestamp] pairs *1/ */
        uint32_t	ipts;	/* IP address [/ timestamp] pairs */
    // TODO(HE7086): flexarr
} __attribute__((__packed__));
