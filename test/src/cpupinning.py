cluster_size = 6 # shares L3 cache
hyperthreads = 2
...
class CpuPinner:
    ...
    def in_cluster(self, cluster: int, offset: int, length: int = 1):
        assert offset + length - 1 < cluster_size # dont use this function, if the cpu range doesnt fit into a cluster
        start = ((cluster * cluster_size) + offset) % self.cores_available()
        end = (start + length - 1) % self.cores_available()
        return f"{start}-{end}"

    def qemu(self, vm_number: int) -> str:
        vm_number = self._vm_number(vm_number)
        return self.in_cluster(vm_number, 0, length=cluster_size)
    ...
    def vmux_runner(self, vm_number: int):
        vm_number = self._vm_number(vm_number)
        overcommittment = int((vm_number) / (self.cores_available() / hyperthreads / cluster_size))
        offset = (overcommittment * 2 + 2) % cluster_size
        return self.in_cluster(vm_number, offset)

    def vmux_rx(self, vm_number: int):
        vm_number = self._vm_number(vm_number)
        overcommittment = int((vm_number) / (self.cores_available() / hyperthreads / cluster_size))
        offset = (overcommittment * 2 + 3) % cluster_size
        return self.in_cluster(vm_number, offset)
